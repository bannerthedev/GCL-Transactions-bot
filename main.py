...
# ---------------- CONFIG (fill these) ----------------
TOKEN = "MTUwMTc1Mjc5NzUxMzc4MTI5Mg.G30Eq8.qUxIqUlPH97tcTH53T6G1FtSZDxiXOXCAQCWXM"  # <--- PUT YOUR REAL TOKEN HERE
GUILD_ID = 1485163135533842464  # your guild id as int

MATCH_TIMES_CHANNEL_ID = 1485170159735013467
ASSIGNMENTS_CHANNEL_ID = 1501788180389822567
TRANSACTIONS_CHANNEL_ID = 1485170894279147721
MATCH_SCORES_CHANNEL_ID = 1485170232929816737
# ----------------------------------------------------
...
GLOBAL_CAPTAIN_ROLE_ID = 1485382094002720808   # captain role ID
GLOBAL_COCAPTAIN_ROLE_ID = 1485382050801254420  # co‑captain role ID
GLOBAL_PLAYER_ROLE_ID = 1485382139074711702    # player role ID
...
async def code(interaction: discord.Interaction, team1: discord.Role, team2: discord.Role):
    ...
    code_value = f"GCL{random.randint(1000, 9999)}"
...
@bot.tree.command(guild=discord.Object(id=GUILD_ID), name="seeding", description="Show current seeding (staff only)")
async def seeding(interaction: discord.Interaction):
    ...
    lines.append("")
    lines.append("Key:")
    lines.append(f"Win = {POINTS_FOR_WIN}pts")
    lines.append(f"Loss = {POINTS_FOR_LOSS}pts")
    lines.append(f"Timecap = {POINTS_FOR_TIMECAP}pts")

    await interaction.response.send_message("\n".join(lines), ephemeral=True)

@bot.tree.command(guild=discord.Object(id=GUILD_ID), name="record_result", description="Record match result and update seeding (staff only)")
@discord.app_commands.describe(team_winner="Winning team name (exact)", team_loser="Losing team name (exact)", timecap="Was it a timecap? (yes/no)")
async def record_result(interaction: discord.Interaction, team_winner: str, team_loser: str, timecap: str = "no"):
    if not is_staff(interaction.user):
        await interaction.response.send_message("Only staff can use this.", ephemeral=True)
        return
    ...
    winner = ensure_team_stats_entry(teams, team_winner.strip())
    loser = ensure_team_stats_entry(teams, team_loser.strip())

    winner["wins"] = int(winner.get("wins", 0)) + 1
    loser["losses"] = int(loser.get("losses", 0)) + 1

    if str(timecap).strip().lower() in ("yes", "y", "true", "1"):
        winner["timecaps"] = int(winner.get("timecaps", 0)) + 1

    recompute_points(winner)
    recompute_points(loser)
    ...
    if tx_ch:
        await tx_ch.send(
            f"Result recorded: {winner['name']} beat {loser['name']} (timecap={timecap}). "
            f"Updated points: {winner['name']} {winner['points']}P, {loser['name']} {loser['points']}P"
        )

    await interaction.response.send_message(
        f"Result recorded. {winner['name']} now has {winner['points']} points.",
        ephemeral=True
    )
...
# ---------- add_player command ----------
@bot.tree.command(guild=discord.Object(id=GUILD_ID), name="add_player", description="Add a player to a team (assign roles)")
...
async def captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"GCL Captain Panel – {team_name}",
        description=desc,
        colour=colour
    )
    ...
    embed.set_footer(text="GCL Transactions Bot")
...
async def co_captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"GCL Co-Captain Panel – {team_name}",
        description=desc,
        colour=colour
    )
    embed.add_field(name="👑 Captain", value=captain.mention if captain else "None", inline=False)
...
@bot.tree.command(
    guild=discord.Object(id=GUILD_ID),
    name="info",
    description="Show information about the GCL Transactions Bot commands"
)
async def info(interaction: discord.Interaction):
    embed = discord.Embed(
        title="GCL Transactions Bot – Command Guide",
        description=(
            "What every command does and who is allowed to use it.\n\n"
            "**Bot Name:** GCL Transactions Bot\n"
            "**Created by:** banner"
        ),
        colour=discord.Colour.blurple()
    )
    ...
    embed.set_footer(text="GCL Transactions Bot • created by banner")
